"""Residual/gated transformer retriever that starts near baseline behavior."""
...
from movie_recsys.modeling.towers import MLP, ItemTower, masked_mean_pool
from movie_recsys.training.config import RetrievalConfig


class ResidualTransformerUserTower(nn.Module):
    def __init__(
        self,
        *,
        num_users: int,
        ...
        initial_transformer_gate: float,
        dropout: float,
    ) -> None:
        super().__init__()
        ...
        self.transformer_gate = nn.Parameter(torch.tensor(float(initial_transformer_gate)))
    ...
    def transformer_gate_alpha(self) -> torch.Tensor:
        return torch.sigmoid(self.transformer_gate)
    ...
    def forward(
        ...
        *,
        ...
        baseline_context = masked_mean_pool(history_emb, history_mask)

        debug: dict[str, object] = {}
        if return_debug:
            transformer_context, attention_weights = self.sequence_encoder(
                history_emb,
                history_mask,
            ...
            transformer_context = self.sequence_encoder(history_emb, history_mask)

        alpha = self.transformer_gate_alpha()
        transformer_delta = transformer_context - baseline_context
        user_seq = baseline_context + alpha * transformer_delta

        user_feat_vec = self.user_features_mlp(user_features)
        merged = torch.cat([user_emb, user_seq, user_feat_vec], dim=1)
        projected = self.proj(merged)
        normalized = nn.functional.normalize(projected, p=2, dim=1)

        if return_debug:
            debug["baseline_context"] = baseline_context
            debug["transformer_context"] = transformer_context
            debug["transformer_gate_alpha"] = alpha.detach()
            debug["user_seq"] = user_seq
...
class ResidualTransformerRetriever(nn.Module):
    def __init__(
        self,
        *,
        ...
        self.user_tower = ResidualTransformerUserTower(
            num_users=num_users,
            ...
            initial_transformer_gate=config.model.initial_transformer_gate,
            dropout=config.model.dropout,
        )
        # Item tower is intentionally the approved baseline implementation.
